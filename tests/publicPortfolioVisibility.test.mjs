...
const conversionSectionsPath = new URL(
  "../app/components/HomeConversionSections.tsx",
  import.meta.url,
);

test("couples portfolio stays out of every public gallery surface", async () => {
  const [dataSource, portfolioSource, homeSource, conversionSource] = await Promise.all([
    readFile(professionalDataPath, "utf8"),
    readFile(portfolioPagePath, "utf8"),
    readFile(homePagePath, "utf8"),
    readFile(conversionSectionsPath, "utf8"),
  ]);
  ...
  assert.doesNotMatch(conversionSource, /portfolio\?category=couples/);
  assert.doesNotMatch(conversionSource, /Couples photography gallery/);
});
