-- Grad guide publishing (2026-07-17): guide_photo now supports guide='grad',
-- targeting the campus-spots guide (/grad-guide/campus-spots). Each
-- location_spots row is a SINGLE image slot (image_url), so unlike the
-- family/couples guide tables this branch REPLACES the spot's current photo.
-- The previous image_url is stashed in published_ref.replaced_image_url so
-- takedown can restore it (takedown.ts only restores while the spot still
-- shows this item's derivative).
--
-- location_key for grad is the location_spots row id (numeric string) —
-- validated structurally in the payload schema, for existence here.
-- Every non-guide branch is byte-identical to migration 20260707000001.
...
begin
  ...
  if v_item.content_type = 'journal_post' then
    ...
    if v_guide not in ('family','couples','grad') then raise exception 'invalid guide %', v_guide; end if;
    select sp.* into v_photo from public.session_photos sp
     where sp.id = (v_item.payload->>'session_photo_id')::uuid
       and sp.photography_session_id = v_session_id;
