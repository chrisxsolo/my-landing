-- Guide-photo captions (couples guide publishing, 2026-07-07): the engine's
-- guide_photo payload now carries an optional AI-drafted `caption` (reviewed
-- and editable in the admin before approval). Both guide tables
-- (family_location_photos, couples_location_photos) have had a `caption`
-- column since creation and both public location templates render it under
-- the photo — the publish RPC just never wrote it.
--
-- This migration re-creates publish_session_content_item so the two
-- guide_photo inserts persist nullif(payload->>'caption',''), exactly like the
-- school_page_photo branch has always done. Every other branch is
-- byte-identical to migration 20260618000001. Backward compatible: items
-- generated before the caption field publish with caption null.
