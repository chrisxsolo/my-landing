-- Republish-after-takedown fix (2026-07-25):
-- 1. school_page_photos: ON CONFLICT DO NOTHING left a taken-down row inactive
--    forever (and dropped the new alt/caption/sort). Now re-activates and
--    refreshes the payload fields.
-- 2. family/couples/portrait guide photos: reusing an existing row matched on
--    location_slug + image_url never re-set published = true, so publishing
--    after a takedown reported success while the photo stayed hidden. The
--    reuse path now re-publishes and carries the newly approved alt/caption.
-- Only those two blocks differ from 20260717000002; the function is otherwise
-- reproduced verbatim.
...
begin
  ...
  if v_item.content_type = 'journal_post' then
    ...
    -- Republish after takedown: the row already exists with active=false, so
    -- DO NOTHING would leave it hidden forever. Re-activate and carry the
    -- newly approved fields.
    on conflict (school_slug, session_photo_id) do update
      set active = true,
          alt_override = excluded.alt_override,
    ...
    if v_guide = 'family' then
      ...
      if v_guide_existing is null then
        ...
        -- Republish after takedown: re-activate the matched row and carry the
        -- newly approved alt/caption.
        update public.family_location_photos
           set published = true,
               alt_text = nullif(v_item.payload->>'alt_text',''),
               caption = nullif(v_item.payload->>'caption','')
         where id = v_guide_existing;
      end if;
      ...
      if v_guide_existing is null then
        ...
        update public.couples_location_photos
           set published = true,
               alt_text = nullif(v_item.payload->>'alt_text',''),
               caption = nullif(v_item.payload->>'caption','')
         where id = v_guide_existing;
      ...
      if v_guide_existing is null then
        ...
        update public.portrait_location_photos
           set published = true,
               alt_text = nullif(v_item.payload->>'alt_text',''),
               caption = nullif(v_item.payload->>'caption','')
         where id = v_guide_existing;
