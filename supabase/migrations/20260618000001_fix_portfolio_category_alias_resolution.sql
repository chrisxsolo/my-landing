-- Fix: portfolio_pick publishing failed with "portfolio category <slug> not
-- found" because the engine speaks the canonical/display category vocabulary
-- (grads, families) while the portfolio_categories table stores the raw slugs
-- (graduation, family). The original lookup matched payload.category against
-- pc.slug directly, so 'grads' / 'families' never resolved.
--
-- This migration re-creates publish_session_content_item so the portfolio_pick
-- branch resolves the engine slug to the real category row — the inverse of
-- PORTFOLIO_CATEGORY_SLUG_ALIASES in lib/photoMetadata.ts — and stores the
-- resolved real slug in portfolio_images.category_slug (preserving the existing
-- invariant that category_slug equals the FK'd category's slug). Only the
-- portfolio_pick branch and one local declaration change; every other branch is
-- byte-identical to migration 20260611000008.
