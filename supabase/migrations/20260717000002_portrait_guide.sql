-- Portrait guide (2026-07-17): the lifestyle/individual portrait guide at
-- /portrait-guide mirrors the family and couples guides — a
-- portrait_location_photos table + public portrait-photos bucket for the
-- location galleries, and a 'portrait' branch in the publish RPC so the
-- content engine can place portrait-session photos onto guide location pages
-- (guide_photo payload guide='portrait', location_key = registry slug in
-- lib/portraitGuide/locations/*).
--
-- Table/bucket/RLS mirror 20260609130000_create_couples_location_photos.sql;
-- the RPC's portrait branch mirrors the couples branch; every other RPC branch
-- is byte-identical to migration 20260717000001. Idempotent: safe to re-run.

-- ── 1. Table ─────────────────────────────────────────────────────────────────

create extension if not exists "pgcrypto";

create table if not exists public.portrait_location_photos (
  ...
  constraint portrait_location_photos_slug_not_blank check (length(trim(location_slug)) > 0),
  constraint portrait_location_photos_image_not_blank check (length(trim(image_url)) > 0),
  constraint portrait_location_photos_sort_order_check check (sort_order >= 0),
  constraint portrait_location_photos_dimensions_check check (
    (width is null and height is null) or (width > 0 and height > 0)
  )
);

drop trigger if exists portrait_location_photos_set_updated_at on public.portrait_location_photos;

create trigger portrait_location_photos_set_updated_at
before update on public.portrait_location_photos
for each row execute function public.set_updated_at();

-- Primary read path: published photos for one location, featured first then order.
create index if not exists portrait_location_photos_location_idx
on public.portrait_location_photos(location_slug, published, featured desc, sort_order);
...
revoke all on public.portrait_location_photos from anon, authenticated;
grant select on public.portrait_location_photos to anon, authenticated;
grant all on public.portrait_location_photos to service_role;

alter table public.portrait_location_photos enable row level security;
alter table public.portrait_location_photos force row level security;

drop policy if exists "Public read published portrait photos" on public.portrait_location_photos;
create policy "Public read published portrait photos"
  on public.portrait_location_photos
  for select
  to anon, authenticated
  using (published = true);

-- Public storage bucket for delivered marketing photos (same model as
-- couples-photos). Uploads/deletes are performed server-side with the service
-- role; public read lets next/image fetch the optimized source.
...
-- ── 2. Publish RPC: accept guide='portrait' ──────────────────────────────────
...
declare
  ...
  v_spot public.location_spots%rowtype;
begin
  ...
  if v_item.content_type = 'journal_post' then
    ...
    if v_guide not in ('family','couples','portrait','grad') then raise exception 'invalid guide %', v_guide; end if;
    select sp.* into v_photo from public.session_photos sp
     where sp.id = (v_item.payload->>'session_photo_id')::uuid
       and sp.photography_session_id = v_session_id;
    ...
    if v_guide = 'family' then
      ...
      v_target_id := v_guide_existing::text;
    elsif v_guide = 'portrait' then
      select pt.id into v_guide_existing from public.portrait_location_photos pt
       where pt.location_slug = v_item.payload->>'location_key'
         and pt.image_url = v_photo.public_derivative_url limit 1;
      if v_guide_existing is null then
        insert into public.portrait_location_photos (location_slug, image_url, alt_text, caption, published, sort_order)
        values (v_item.payload->>'location_key', v_photo.public_derivative_url,
                nullif(v_item.payload->>'alt_text',''), nullif(v_item.payload->>'caption',''), true,
                coalesce((select max(sort_order)+1 from public.portrait_location_photos
                          where location_slug = v_item.payload->>'location_key'), 0))
        returning id into v_guide_existing;
      end if;
      v_target_type := 'portrait_location_photo';
      v_target_id := v_guide_existing::text;
    else
      -- grad: the campus spot is a single image slot. Lock the row, remember
      -- what it showed, and replace it. published_ref.replaced_image_url is
      -- the takedown restore point.
      select ls.* into v_spot from public.location_spots ls
       where ls.id = (v_item.payload->>'location_key')::bigint
       for update;
      if not found then
        raise exception 'campus spot % not found', v_item.payload->>'location_key';
      end if;
      v_ref := jsonb_build_object(
        'replaced_image_url', v_spot.image_url,
        'spot_name', v_spot.name,
        'school_id', v_spot.school_id);
      update public.location_spots
         set image_url = v_photo.public_derivative_url
       where id = v_spot.id;
      v_target_type := 'grad_spot_photo';
      v_target_id := v_spot.id::text;
    end if;
