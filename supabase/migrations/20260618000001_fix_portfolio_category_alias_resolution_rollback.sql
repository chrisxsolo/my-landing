-- Rollback for 20260618000001: restore publish_session_content_item to the
-- 20260611000008 definition (portfolio_pick matches payload.category against
-- pc.slug directly, with no engine→raw slug resolution). NOTE: rolling back
-- re-introduces the "portfolio category grads not found" failure for
-- portfolio_pick items whose category is grads/families.
