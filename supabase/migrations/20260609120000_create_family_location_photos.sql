-- Family-guide location galleries.
-- One row per uploaded photograph, associated to a location by its slug (matches
-- FamilyLocationData.slug in lib/familyGuide/locations/*).
--
-- Access model (RLS):
...
-- Image bytes live in a PUBLIC storage bucket (family-photos) so the delivered
-- image_url renders in the browser via next/image, the same way grad-photos does.
...
create table if not exists public.family_location_photos (
  ...
  constraint family_location_photos_slug_not_blank check (length(trim(location_slug)) > 0),
  constraint family_location_photos_image_not_blank check (length(trim(image_url)) > 0),
  constraint family_location_photos_sort_order_check check (sort_order >= 0),
  constraint family_location_photos_dimensions_check check (
    (width is null and height is null) or (width > 0 and height > 0)
  )
);

drop trigger if exists family_location_photos_set_updated_at on public.family_location_photos;
...
create index if not exists family_location_photos_location_idx
on public.family_location_photos(location_slug, published, featured desc, sort_order);
...
revoke all on public.family_location_photos from anon, authenticated;
grant select on public.family_location_photos to anon, authenticated;
...
drop policy if exists "Public read published family photos" on public.family_location_photos;
create policy "Public read published family photos"
  on public.family_location_photos
  for select
  to anon, authenticated
  using (published = true);

-- Public storage bucket for delivered marketing photos (same model as grad-photos).
