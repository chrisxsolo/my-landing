// Shared, server-rendered template for every individual portrait-location page.
// Receives a typed PortraitLocationData config + server-fetched photos, and
// renders all sections (hero, why, variety, time, seasons, wardrobe, posing,
...
// Mirrors the couples location template (shared fg-* design system).

import Link from "next/link";
import OptimizedPhoto from "@/app/components/OptimizedPhoto";
import { buildBreadcrumbJsonLd } from "@/lib/breadcrumbs";
import { PORTRAIT_GUIDE_CSS } from "@/lib/portraitGuide/styles";
import { locationDisplayName, type PortraitLocationData } from "@/lib/portraitGuide/types";
import type { PortraitPhoto } from "@/lib/portraitGuide/photos";
...
function contactHref(name: string) {
  return `/contact?sessionType=Individual+Portrait&location=${encodeURIComponent(name)}`;
}

const relatedLinks = [
  { href: "/portrait-guide", title: "Lifestyle Portrait Guide", desc: "Outfits, prep, light, and planning in one place." },
  { href: "/portrait-guide/locations", title: "All Portrait Photo Locations", desc: "Compare San Francisco portrait spots." },
  { href: "/portrait-guide/what-to-wear", title: "What to Wear", desc: "Colors, layers, and branding outfits." },
  { href: "/portrait-guide/how-to-prepare", title: "How to Prepare", desc: "Show up relaxed and ready." },
  { href: "/portrait-guide/what-to-expect", title: "What to Expect", desc: "How a guided session flows." },
...
export default function PortraitLocationTemplate({
  data,
  photos,
}: {
  data: PortraitLocationData;
  ...
  const breadcrumbLd = buildBreadcrumbJsonLd([
    ...
    { name: "Portrait Photo Locations", path: "/portrait-guide/locations" },
  ...
  const serviceLd = {
    ...
    serviceType: [`${data.name} portrait photography`],
    ...
    <main className="fg-page">
      ...
      <nav className="fg-crumbs fg-shell" aria-label="Breadcrumb">
        <ol>
          ...
          <li><Link href="/portrait-guide/locations">Locations</Link><span className="fg-crumb-sep" aria-hidden="true">/</span></li>
      ...
      <header className="fg-section fg-lhero">
        <div className="fg-shell">
          <p className="fg-kicker"><span className="fg-kicker-dot" /> {data.region} Portrait Photography</p>
          ...
          <div className="fg-actions">
            ...
            <Link href="/portrait-guide/locations" className="fg-btn fg-btn--ghost">All portrait locations</Link>
          </div>
          ...
          <div className="fg-lhero-media">
            {hero ? (
              ...
              <div className="fg-lhero-ph"><span aria-hidden="true">📷</span><p>A featured portrait from this location appears here soon.</p></div>
            )}
          </div>
        ...
        <div className="fg-shell">
          <div className="fg-prose">
            <div className="fg-topic" data-reveal>
              <h2>Why {name} Works for Portraits</h2>
            ...
            <div className="fg-topic" data-reveal>
              ...
              <p>
                ...
                <Link href="/portrait-guide/what-to-wear" className="fg-inline-link">what-to-wear guide</Link>.
              </p>
            </div>

            <div className="fg-topic" data-reveal>
              <h2>Posing and Direction</h2>
              <Paragraphs items={data.posingDirection} />
            </div>
      ...
      <section className="fg-section">
        <div className="fg-shell">
          ...
          <h2 className="fg-sec-title" data-reveal>{name} portraits</h2>
          {photos.length > 0 ? (
            <div className="fg-gallery">
              {photos.map((photo) => (
                <div key={photo.id} className="fg-photo" data-reveal data-delay="1">
                  <OptimizedPhoto
                    src={photo.image_url}
                    alt={photo.alt_text || `${name} portrait photography by soloxsnaps`}
            ...
            <div className="fg-empty" data-reveal>
              ...
              <p style={{ fontWeight: 760, color: "var(--ink)" }}>
                {name} portrait sessions will appear here as soon as Chris adds them.
              </p>
            </div>
          )}
        </div>
