...
export default function AvailabilityPage() {
  ...
  const availCount=dates.filter(d=>{const[y,m]=d.date.split("-").map(Number);return d.status==="available"&&y===year&&m===month+1}).length;
  ...
  const CAL_STYLES = `
    ...
    .day-avail{background:${C.p1_10};border:1.5px solid ${C.p1_25};cursor:pointer;}
    .day-avail:hover{box-shadow:0 4px 16px ${C.p1_20};}
    .day-booked{background:rgba(0,0,0,0.04);border:1.5px solid rgba(0,0,0,0.08);cursor:default;}
    .day-hold{background:${C.p3_10};border:1.5px solid ${C.p3_30};cursor:pointer;}
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden" style={{ background: C.page }}>
      ...
      <section className="relative overflow-hidden px-6 pt-16 pb-14 border-b" style={{ borderColor: C.borderSubtle }}>
        ...
        <div className="blob1 absolute rounded-full pointer-events-none" style={{width:500,height:500,top:-130,left:-110,background:C.blob1}}/>
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 pointer-events-none hidden sm:block" style={{opacity:0.4,animation:"fadeIn 1s 0.6s ease both"}}>
          <svg width="100" height="200" viewBox="0 0 100 200" fill="none">
            <defs><linearGradient id="avsg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.p1}/><stop offset="50%" stopColor={C.p2}/><stop offset="100%" stopColor={C.p3}/></linearGradient></defs>
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <div className="afu1 inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5" style={{background:C.p1_08,border:`1px solid ${C.p1_20}`}}>
            ...
            <p className="text-xs font-bold tracking-[0.12em] uppercase" style={{color:C.p1}}>Shoot Availability</p>
          </div>
          <h1 className="afu2 text-5xl sm:text-6xl font-black tracking-tight leading-tight text-slate-900 mb-2">When I'm</h1>
          <p className="afu3 text-5xl sm:text-6xl font-light italic tracking-tight text-slate-900 mb-6">
            <span style={C.text}>available.</span>
            <span className="cblink inline-block w-[3px] h-[44px] sm:h-[52px] ml-1.5 rounded-sm align-middle" style={{background:C.grad12}}/>
          </p>
          <p className="afu4 text-lg text-slate-600 font-light leading-relaxed max-w-md mx-auto">Grad season books up fast. Check my open dates below and reach out to lock one in.</p>
        </div>
      </section>
      ...
      <section className="px-6 py-14">
        <div className="max-w-2xl mx-auto">
          <div className="rounded-2xl overflow-hidden" style={{background:C.surfaceStrong,border:`1px solid ${C.borderWarm}`,boxShadow:C.shadowWarmLg}}>
            ...
            {/* Day labels */}
            <div className="grid grid-cols-7 border-b border-black/[0.06]" style={{background:C.p1_04}}>
            ...
            <div className="p-4" style={{ background: C.surfaceSoft }}>
              {loading ? (
                ...
                <div className="grid grid-cols-7 gap-1.5">
                  {[...Array(firstDay)].map((_,i)=><div key={`e${i}`} className="aspect-square rounded-xl day-btn day-empty"/>)}
                  {[...Array(daysInMonth)].map((_,i)=>{
                    ...
                    if(isPast)cls+="day-empty opacity-30 ";
                    else if(entry?.status==="available")cls+="day-avail ";
                    else if(entry?.status==="booked")cls+="day-booked ";
                    else if(entry?.status==="hold")cls+="day-hold ";
                    else cls+="day-empty ";
                    ...
                    return(
                      <button key={day} className={cls}
                        style={isToday?{outline:`2px solid ${C.p1}`,outlineOffset:"1px"}:isSel?{outline:`3px solid ${C.p2}`,outlineOffset:"1px"}:{}}
                        onClick={()=>{if(entry&&!isPast)setSelected(isSel?null:entry)}}>
                        <span className="text-xs font-black leading-none" style={{color:textColor}}>{day}</span>
                        {entry?.status==="available"&&!isPast&&<span className="w-1 h-1 rounded-full mt-0.5" style={{background:C.grad12}}/>}
                        {entry?.status==="hold"&&!isPast&&<span className="w-1 h-1 rounded-full mt-0.5" style={{background:C.p3}}/>}
                      </button>
                    );
                  })}
                </div>
            ...
            <div className="px-4 pb-4 flex flex-wrap gap-4 justify-center">
              {[{label:"Available",dot:C.grad12},{label:"On hold",dot:C.p3},{label:"Booked",dot:"#e2e8f0"}].map(l=>(
          ...
          {selected&&(
            <div className="mt-4 rounded-2xl p-5 relative overflow-hidden" style={{border:`1.5px solid ${C.p1_25}`,background:`linear-gradient(135deg,${C.p1_06},${C.p2_06})`}}>
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-white text-sm font-black" style={{background:C.grad12}}>
                  {new Date(selected.date+"T12:00:00").getDate()}
                </div>
                <div className="flex-1">
                  <p className="font-black text-slate-900 text-sm">{new Date(selected.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</p>
                  <p className="text-xs font-semibold mt-0.5" style={{color:selected.status==="available"?C.p1:C.p3}}>
          ...
          <div className="mt-8 rounded-2xl p-8 text-white text-center relative overflow-hidden" style={{background:C.grad}}>
            <div className="absolute inset-0 pointer-events-none" style={C.ctaGrid}/>
            <div className="absolute top-3 right-4 opacity-15 pointer-events-none"><svg width="40" height="64" viewBox="0 0 40 64" fill="none"><path d="M20 4 C30 14,10 24,20 36 C30 48,10 56,20 62" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round"/></svg></div>
            ...
            <a href="https://www.soloxsnaps.com/contact/" className="btn-lift relative inline-block px-7 py-3 rounded-full bg-white font-bold text-sm" style={{color:C.p1}}>Book your shoot →</a>
          </div>
        </div>
      </section>
