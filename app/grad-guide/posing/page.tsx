      ...
      <section className="relative overflow-hidden px-6 pt-14 pb-10">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${t.pill} mb-4`}>
            ...
            <p className={`text-xs font-semibold tracking-[0.12em] uppercase ${t.label}`}>01 — Posing Guide</p>
          </div>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {loading ? (
            ...
            poses.map((pose, index) => (
              <div key={pose.id} className={`bg-white rounded-2xl border border-black/[0.08] overflow-hidden transition-all duration-200 ${t.hover}`}>
                <div className={`grid ${pose.image_url ? "sm:grid-cols-2" : "grid-cols-1"}`}>
                  ...
                  {!pose.image_url && (
                    <div className={`hidden sm:flex aspect-square sm:aspect-auto bg-gradient-to-br ${t.blob} opacity-30 items-center justify-center`}>
                  ...
                  <div className="p-7 flex flex-col justify-center">
                    <div className="mb-3">
                      <span className={`text-xs font-bold tracking-widest ${t.label} bg-gradient-to-r ${t.pill} px-2.5 py-1 rounded-full`}>
                        POSE {String(index + 1).padStart(2, "0")}
                      </span>
                    </div>
                    <h2 className="text-xl font-bold text-slate-900 mb-3 leading-tight">{pose.title}</h2>
                    <p className="text-slate-500 text-sm leading-relaxed">{pose.instructions}</p>
                  </div>
                </div>
              </div>
            ))
      ...
      <section className="px-6 pb-20">
        <div className="max-w-3xl mx-auto">
          <div className={`bg-gradient-to-br ${t.cta} rounded-2xl p-8 text-white text-center`}>
            ...
            <p className="text-white/70 mb-6 text-sm">Save these poses on your phone so you can reference them day-of.</p>
            <div className="flex flex-wrap justify-center gap-3">
              <Link href="/grad-guide/what-to-wear" className={`px-5 py-2.5 rounded-full bg-white ${t.ctaText} font-semibold text-sm hover:bg-white/90 transition-colors`}>
                Next: What to Wear →
              </Link>
              <a href="https://www.soloxsnaps.com/contact/" className="px-5 py-2.5 rounded-full bg-white/10 text-white font-semibold text-sm hover:bg-white/20 transition-colors border border-white/20">
                Book a shoot
              </a>
            </div>
          </div>
