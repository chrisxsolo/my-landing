// Admin API for portrait-guide location galleries (portrait_location_photos).
...
// object in the portrait-photos bucket, so deleting a photo never leaves an
// orphan. Mirrors app/api/admin/couples-photos/route.ts.

import { NextRequest, NextResponse } from "next/server";
import { createSupabaseAdminClient } from "@/lib/supabaseAdmin";
import { requireAdmin } from "@/lib/requireAdmin";
import {
  PORTRAIT_PHOTOS_BUCKET,
  PORTRAIT_LOCATION_PHOTOS_TABLE,
  isValidPortraitLocationSlug,
  validateAltText,
} from "@/lib/portraitPhotosAdmin";
...
function jsonError(error: unknown, fallback: string, status = 500) {
  console.error(`[admin/portrait-photos] ${fallback}`, error);
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!isValidPortraitLocationSlug(slug)) return NextResponse.json({ photos: [] });
    ...
    return jsonError(error, "Failed to load portrait photos.");
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!isValidPortraitLocationSlug(slug)) throw new Error("Unknown portrait location.");
    ...
    const supabase = createSupabaseAdminClient();
    const image_url = supabase.storage.from(PORTRAIT_PHOTOS_BUCKET).getPublicUrl(storagePath).data.publicUrl;
    ...
    try {
      if (replaceId) {
        ...
        if (existing.storage_path) {
          const rm = await supabase.storage.from(PORTRAIT_PHOTOS_BUCKET).remove([existing.storage_path]);
          if (rm.error) console.error("[admin/portrait-photos] old object cleanup failed", rm.error);
        }
      ...
      const rm = await supabase.storage.from(PORTRAIT_PHOTOS_BUCKET).remove([storagePath]);
      if (rm.error) console.error("[admin/portrait-photos] upload rollback failed", rm.error);
      throw error;
    }
  } catch (error) {
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (action === "setHero") {
      const slug = body.location_slug;
      const id = body.id;
      if (!isValidPortraitLocationSlug(slug) || typeof id !== "string") throw new Error("Invalid hero selection.");
      // A single hero: feature the chosen photo, unfeature the rest in this location.
      const clear = await supabase.from(PORTRAIT_LOCATION_PHOTOS_TABLE).update({ featured: false }).eq("location_slug", slug);
      if (clear.error) throw clear.error;
      const { error } = await supabase.from(PORTRAIT_LOCATION_PHOTOS_TABLE).update({ featured: true }).eq("id", id).eq("location_slug", slug);
      if (error) throw error;
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const { error: deleteError } = await supabase.from(PORTRAIT_LOCATION_PHOTOS_TABLE).delete().eq("id", id);
    if (deleteError) throw deleteError;

    let cleanupWarning: string | null = null;
    if (row.storage_path) {
      const { error } = await supabase.storage.from(PORTRAIT_PHOTOS_BUCKET).remove([row.storage_path]);
      if (error) {
        cleanupWarning = "Record deleted, but the storage file needs manual cleanup.";
        console.error("[admin/portrait-photos] delete cleanup failed", error);
      }
    }
