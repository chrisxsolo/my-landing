...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const supabase = createSupabaseAdminClient();
    const image_url = supabase.storage.from(COUPLES_PHOTOS_BUCKET).getPublicUrl(storagePath).data.publicUrl;
    ...
    try {
      ...
      const rm = await supabase.storage.from(COUPLES_PHOTOS_BUCKET).remove([storagePath]);
