...
import { detectThreadDetails } from "@/lib/confirmationThreadScan";
import { resolveConfirmationPricing } from "@/lib/confirmationPricing";
...
export async function POST(req: NextRequest) {
  ...
  if (mode === "send" && edited_html) {
    ...
    // Read the thread for the confirmed date and, when no payment ever reached
    // the ledger, the deposit the client sent and the price that was quoted.
    ...
    // Price the shoot: quoted price if there is one, otherwise the rate card.
    // With no recorded payment anywhere, the policy retainer stands in.
    const pricing = resolveConfirmationPricing({
      inquiry: inq,
      recordedAmount: amount,
      threadAmount: details.paidAmount,
      threadTotal: details.quotedTotal,
    });

    html = buildPaymentConfirmationHtml({
      name:               inq.name,
      sessionType:        inq.session_type,
      confirmedDateLabel: details.confirmedDateLabel,
      amount:             pricing.amount,
      method:             method || details.method,
      invoice,
      sessionTotal:       pricing.sessionTotal,
      balanceDue:         pricing.balanceDue,
      balanceNote:        pricing.balanceNote,
      siteUrl,
